...
import { useDashboardMetrics, useAgentStatus, useHealthCheck } from "@/hooks/use-api"
...
export default function OverviewPage() {
  ...
  const { healthy, loading: healthLoading, error: healthError } = useHealthCheck()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent className="relative space-y-3">
            <div className="space-y-1">
              <div className="text-3xl font-bold text-white">
                {metricsLoading ? "Loading..." : metrics ? `₹${(metrics.total_disbursed / 10000000).toFixed(1)}Cr` : "₹8.4Cr"}
              </div>
              <div className="text-xs text-gray-400">
                {metricsLoading ? "Loading..." : metrics ? `${metrics.loans_processed.toLocaleString()} transactions` : "2,847 transactions"}
              </div>
        ...
        <Card className="glass-card glass-success border">
          ...
          <CardContent className="relative space-y-3">
            <div className="space-y-1">
              <div className="text-3xl font-bold text-white">
                {metricsLoading ? "Loading..." : metrics ? `${metrics.success_rate}%` : "98.7%"}
              </div>
              <div className="text-xs text-gray-400">
                {metricsLoading ? "Loading..." : metrics ? 
                  `${Math.round(metrics.loans_processed * metrics.success_rate / 100).toLocaleString()} success / ${Math.round(metrics.loans_processed * (100 - metrics.success_rate) / 100).toLocaleString()} failed` : 
                  "2,810 success / 37 failed"
                }
              </div>
              <div className="flex items-center gap-1 text-xs text-blue-400">
                <TrendingUp className="h-3 w-3" />
        ...
        <Card className="glass-card glass-warning border">
          ...
          <CardContent className="relative space-y-3">
            <div className="space-y-1">
              <div className="text-3xl font-bold text-white">
                {metricsLoading ? "Loading..." : metrics ? metrics.batches_awaiting : "24"}
              </div>
              <div className="text-xs text-gray-400">
                {metricsLoading ? "Loading..." : metrics ? `${Math.ceil(metrics.batches_awaiting / 2)} high priority` : "12 high priority"}
              </div>
      ...
      <div className="space-y-4">
        ...
        <Card className="">
          <CardHeader>
            <CardTitle className="">Live Funnel</CardTitle>
            <p className="text-sm text-muted-foreground">Transaction flow stages</p>
          </CardHeader>
          <CardContent>
        ...
        <Card className="">
          <CardHeader>
            <CardTitle className="">SLA Breaches</CardTitle>
            <p className="text-sm text-muted-foreground">Breach counts over time</p>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="">Recent Events</CardTitle>
        ...
        <Card className="">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="">Open Tasks</CardTitle>
            <span className="text-sm text-muted-foreground">28 overdue</span>
          </CardHeader>
          <CardContent>
